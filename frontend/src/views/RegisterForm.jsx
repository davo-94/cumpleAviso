...
export default function RegisterForm({ showToast }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const colab = await api.createColaborador(data);
      if (fotoInputRef.current?.files[0]) {
        await api.uploadFoto(colab.id, fotoInputRef.current.files[0]);
      }
    ...
    <div className="max-w-lg mx-auto bg-white rounded-xl shadow-md p-6 border-t-4 border-blue-500">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <label htmlFor="avisar" className="text-sm text-gray-700">
            Autoriza aviso a la empresa el día de su cumpleaños
          </label>
        </div>
